...
recent = date(2014, 4, 20)
dates = [recent]
old =recent
stop = date(2008, 10, 19)
...
# List of URLs
urls = [
    f"https://www.nytimes.com/books/best-sellers/{i}/picture-books/"
...
for url in urls:
    ...
    search = soup.find_all(class_="css-114t425")
    if search == None:
        continue
    print(search)
    ...
    for item in all_links:
        #print(item)
        if item[0:15] == "http://www.amaz":
            #print("yes")
            ama_links.append(item)
        if item[0:15] == "https://www.ama":
            ama_links.append(item)
    #print(ama_links)
    print(len(titles))
    print(len(ama_links))
...
nyt_monthly_mg_stats_table = pd.DataFrame({"rank": rank, "title": titles, "author": author, "publisher": publisher, "date": final_dates, "amazon links": ama_links})
print(nyt_monthly_mg_stats_table)

#### PT 4 EXPORT

nyt_monthly_mg_stats_table.to_csv(r'NYT_WEEKLY0814.csv')